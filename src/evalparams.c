...
double evaluation_double(const Position* pos, const EvalParamsDouble* params, const MagicData* magic) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        int type = piece % 6;
        int mirrored_sq = (side == WHITE) ? sq : sq ^ 56;
    ...
    double score = (mg * phase + eg * (24.0 - phase)) / 24.0;
    return score;
}

void set_default_evalparams(EvalParams* p) {
    static const int mg[6] = {100, 337, 365, 477, 1025, 0};
    static const int eg[6] = {94, 281, 297, 512, 936, 0};
    ...
    memcpy(p->mg_value, mg, sizeof(mg));
    memcpy(p->eg_value, eg, sizeof(eg));
